...
#define DBGFILE "/tmp/generic-device.txt"
#define INPUT_CONTROL_SIGNALS
#define INPUT_GAINS_SIGNALS stateGainsSIN << velocityGainsSIN << torqueGainsSIN
...
DYNAMICGRAPH_FACTORY_ENTITY_PLUGIN(GenericDevice, "GenericDevice");
const double GenericDevice::TIMESTEP_DEFAULT = 0.001;
...
GenericDevice::GenericDevice( const std::string& n )
  : Entity(n)
  , timestep_(TIMESTEP_DEFAULT)
  , control_(6)
  , sanityCheck_(true)
  , stateSIN( NULL, "GenericDevice(" + n + ")::input(double)::state" )
  , velocitySIN( NULL, "GenericDevice(" + n + ")::input(double)::velocity" )
  , torqueSIN( NULL, "GenericDevice(" + n + ")::input(double)::torque" )
  , currentSIN( NULL, "GenericDevice(" + n + ")::input(double)::current" )
  , stateGainsSIN( NULL, "GenericDevice(" + n + ")::input(double)::stateGains" )
  , velocityGainsSIN( NULL, "GenericDevice(" + n + ")::input(double)::velocityGains" )
  , torqueGainsSIN( NULL, "GenericDevice(" + n + ")::input(double)::torqueGains" )
  , motorcontrolSOUT_(boost::bind(&GenericDevice::getControl, this, _1, _2),
                      INPUT_CONTROL_SIGNALS(0),
                      "GenericDevice(" + n + ")::output(vector)::motorcontrol" )
  , robotState_("GenericDevice(" + n + ")::output(vector)::robotState")
  , robotVelocity_("GenericDevice(" + n + ")::output(vector)::robotVelocity")
  , forcesSOUT_(0)
  , imuSOUT_(0)
  , pseudoTorqueSOUT_(0)
  , temperatureSOUT_(0)
...

{
  ...
  /* --- Commands --- */
  {
    ...
    addCommand("setSoTControlType",
               command::makeCommandVoid2(*this, &GenericDevice::setSoTControlType,
    ...
    addCommand("setHWControlType",
               command::makeCommandVoid2(*this, &GenericDevice::setHWControlType,
    ...
    addCommand("setSanityCheck",
               new command::Setter<GenericDevice, bool>
               (*this, &GenericDevice::setSanityCheck, docstring));
...
void GenericDevice::RegisterSignals() {
...
void GenericDevice::setSanityCheck(const bool & enableCheck) {
  sanityCheck_ = enableCheck;
}

void GenericDevice::setControlType(const std::string &strCtrlType, ControlType &aCtrlType) {
  for (int j = 0; j < 2; j++) {
...
void GenericDevice::setSoTControlType(const std::string &jointNames, const std::string &strCtrlType) {
  setControlType(strCtrlType, jointDevices_[jointNames].SoTcontrol);
}

void GenericDevice::setHWControlType(const std::string &jointNames, const std::string &strCtrlType) {
  setControlType(strCtrlType, jointDevices_[jointNames].HWcontrol);
}

void GenericDevice::setControlPos(const std::string &jointName, const unsigned & index) {
  jointDevices_[jointName].control_index = index;
}


void GenericDevice::setURDFModel(const std::string &aURDFModel) {
...
void GenericDevice::increment(const int& time) {
  ...
  /* Create control from the input vectors. */
  createControlVector(time);
...
void GenericDevice::createControlVector(const int& time) {
  /* Force the recomputation of the input vectors. */
  Vector state;
  Vector vel;
  Vector torque;
  Vector current;
  ...
  for (it_control_type  = jointDevices_.begin();
    ...
    if (u_index != -1) {
      // POSITION -> stateSIN + Check Position limits.
      if (it_control_type->second.SoTcontrol == POSITION) {
        control_[lctl_index] = state[lctl_index];

        if (urdf_joints_[u_index]->limits) {
      ...
      // TORQUE -> torqueSIN + Check Torque limits.
      else if (it_control_type->second.SoTcontrol == TORQUE) {
        control_[lctl_index] = torque[lctl_index];
      ...
      // VELOCITY -> velSIN
        ...
        control_[lctl_index] = vel[lctl_index];
      }
      // CURRENT -> currentSIN
      else if (it_control_type->second.SoTcontrol == CURRENT) {
        control_[lctl_index] = current[lctl_index];
      }
    }
  }
  if (sanityCheck_ && control_.hasNaN()) {
    dgRTLOG () << "GenericDevice::updateControl: Control has NaN values: " << '\n'
               << control_.transpose() << '\n';
    return;
  }
}

int GenericDevice::ParseYAMLString(const std::string & aYamlString) {
...
int GenericDevice::ParseYAMLJointSensor(YAML::Node &aJointSensor) {
...
int GenericDevice::ParseYAMLMapHardwareControlMode(YAML::Node & map_control_mode) {
  if (debug_mode_ > 1) {
  ...
  if (map_control_mode.size() == 0) {
    ...
    for (it_control_type  = jointDevices_.begin();
      ...
      control_types_.push_back(value);
    }
    if (debug_mode_ > 1) {
...
/* Sensor signals */
int GenericDevice::ParseYAMLSensors(YAML::Node &map_sensors) {
  if (map_sensors.IsNull()) {
    std::cerr << "GenericDevice::ParseYAMLString: No sensor detected in YamlString "  << std::endl;
    return -1;
  }
...
void GenericDevice::CreateAForceSignal(const std::string & force_sensor_name) {
  ...
  /* --- SIGNALS --- */
  aForceSOUT_ = new Signal<Vector, int>("GenericDevice(" + getName() + ")::output(vector6)::" +
                                        force_sensor_name);
  forcesSOUT_.push_back(aForceSOUT_);
  signalRegistration(*aForceSOUT_);
}

void GenericDevice::CreateAnImuSignal(const std::string &imu_sensor_name) {
  IMUSOUT * anImuSOUT_;
  /* --- SIGNALS --- */
...
int GenericDevice::UpdateSignals() {
  if ((torque_index_ != 0) && (pseudoTorqueSOUT_ != 0)) {
    pseudoTorqueSOUT_ = new Signal<Vector, int>("GenericDevice(" + getName() + ")::output(vector)::ptorque");
  }
  // if ((force_index_!=0) && (forcesSOUT_.size()!=0))
  // {
  //   forcesSOUT_ = new Signal<Vector, int>("GenericDevice("+getName()+")::output(vector)::forces");
  // }
  if ((current_index_ != 0) && (currentsSOUT_ != 0)) {
    currentsSOUT_ = new Signal<Vector, int>("GenericDevice(" + getName() + ")::output(vector)::currents");
  }

  if ((temperature_index_ != 0) && (temperatureSOUT_ != 0)) {
    temperatureSOUT_ = new Signal<Vector, int>("GenericDevice(" + getName() + ")::output(vector)::temperatures");
  }

  if ((motor_angle_index_ != 0) && (motor_anglesSOUT_ != 0)) {
    motor_anglesSOUT_ = new Signal<Vector, int>("GenericDevice(" + getName() + ")::output(vector)::motor_angles");
  }

  if ((joint_angle_index_ != 0) && (joint_anglesSOUT_ != 0)) {
    joint_anglesSOUT_ = new Signal<Vector, int>("GenericDevice(" + getName() + ")::output(vector)::joint_angles");
  }
...
void GenericDevice::display ( std::ostream& os ) const {
  os << name << ": " << control_ << endl;
}

/* Helpers for the controller */
void GenericDevice::setSensorsForce(map<string, dgsot::SensorValues> &SensorsIn, int t) {
...
void GenericDevice::setSensorsIMU(map<string, dgsot::SensorValues> &SensorsIn, int t) {
...
void GenericDevice::setSensorsEncoders(map<string, dgsot::SensorValues> &SensorsIn, int t) {
...
void GenericDevice::setSensorsVelocities(map<string, dgsot::SensorValues> &SensorsIn, int t) {
...
void GenericDevice::setSensorsTorquesCurrents(map<string, dgsot::SensorValues> &SensorsIn, int t) {
...
void GenericDevice::setSensorsGains(map<string, dgsot::SensorValues> &SensorsIn, int t) {
...
void GenericDevice::setSensors(map<string, dgsot::SensorValues> &SensorsIn) {
  sotDEBUGIN(25) ;
...
void GenericDevice::setupSetSensors(map<string, dgsot::SensorValues> &SensorsIn) {
  setSensors (SensorsIn);
}

void GenericDevice::nominalSetSensors(map<string, dgsot::SensorValues> &SensorsIn) {
  setSensors (SensorsIn);
}


void GenericDevice::cleanupSetSensors(map<string, dgsot::SensorValues> &SensorsIn) {
  setSensors (SensorsIn);
}

dg::Vector& GenericDevice::getControl(dg::Vector &controlOut, const int& t) {
  ODEBUG5FULL("start");
  sotDEBUGIN(25);
