 ...
 public:
  ...
  void checkTaskOutput() {
    ...
    for (int i = 0; i < expectedTaskOutput_.size(); ++i)
      taskOutput[i] = task_.taskSOUT.accessCopy()[static_cast<std::size_t>(i)].getSingleBound();
...
Vector toVector(const std::vector<MultiBound> &in) {
  Vector out(in.size());
  for (int i = 0; i < (int)in.size(); ++i) out[i] = in[static_cast<std::size_t>(i)].getSingleBound();
  return out;
}
